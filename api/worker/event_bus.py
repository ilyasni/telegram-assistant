...
from prometheus_client import Counter as PromCounter, Gauge as PromGauge, Histogram as PromHistogram, REGISTRY
...
def _get_or_create_metric(factory, name, documentation, *args, **kwargs):
    ...
    if collectors and name in collectors:
        return collectors[name]
    return factory(name, documentation, *args, **kwargs)


def Counter(name, documentation, *args, **kwargs):
    return _get_or_create_metric(PromCounter, name, documentation, *args, **kwargs)


def Gauge(name, documentation, *args, **kwargs):
    return _get_or_create_metric(PromGauge, name, documentation, *args, **kwargs)


def Histogram(name, documentation, *args, **kwargs):
    return _get_or_create_metric(PromHistogram, name, documentation, *args, **kwargs)
...
class DigestGenerateEvent(BaseEvent):
    ...
    context: Literal["channels", "group"] = "channels"
    group_id: Optional[str] = None
    group_window_id: Optional[str] = None
    window_size_hours: Optional[int] = None
    delivery_channel: Optional[str] = "telegram"
    delivery_format: Optional[str] = "markdown"


class DigestContextPreparedEvent(BaseEvent):
    """Событие: подготовка контекста для группового дайджеста."""
    tenant_id: str
    group_id: str
    window_id: str
    message_total: int
