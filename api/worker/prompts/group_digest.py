...
def role_classifier_prompt_v1() -> ChatPromptTemplate:
    ...
    system = dedent(
        ...
        - ВАЖНО: НЕ учитывай пересылки (forwarded messages) при определении ролей. Пересылки - это не оригинальный контент участника, а перепост чужих сообщений.
          Учитывай только оригинальные сообщения участника. Если participant_stats показывает forwarded_count > 0, это значит, что участник пересылал сообщения, но это не его активность.
          Для оценки вклада используй только message_count (количество оригинальных сообщений), игнорируй forwarded_count.
...
def topic_synthesizer_prompt_v1() -> ChatPromptTemplate:
    ...
    system = dedent(
        ...
        КРИТИЧЕСКИ ВАЖНО:
        - Всегда верни минимум 1 тему, даже если обсуждение хаотичное.
        - Извлеки ВСЕ значимые темы обсуждения, не ограничивайся количеством. Если тем больше 5, верни все значимые темы.
        - Запрещается возвращать пустой массив topics.
        - НЕ ПРОПУСКАЙ темы: если в semantic_units упоминаются разные предметы обсуждения (VPN, безопасность, подключение, сеть, названия улиц, памятники и т.д.) - каждая значимая тема должна быть выделена отдельно.
        - Не объединяй разные темы в одну, даже если они кажутся похожими. Каждая значимая тема должна быть отдельной записью в массиве topics.
        
        Требования:
        - Все темы отсортированы по приоритету (critical > high > medium > low) и msg_count.
        - КРИТИЧЕСКИ ВАЖНО: Если в semantic_units или в сообщениях упоминается VPN, ВПН, виртуальная частная сеть, VPN-сервис, VPN-подключение, безопасность сети - обязательно выдели отдельную тему про VPN.
        - Для каждой темы обязательно укажи:
          title — должен содержать КОНКРЕТНЫЕ ключевые слова из сообщений. ЗАПРЕЩЕНО использовать общие слова типа "Вопрос", "Решение", "Знаю", "Только", "Проблема", "Тема", "Обсуждение" и другие служебные слова как название темы. Название должно быть информативным и отражать суть обсуждения (например, "Происхождение названия памятника", "VPN для обхода блокировок", "Названия улиц Воронежа", а НЕ "Вопрос" или "Знаю").
...
def digest_composer_prompt_v2() -> ChatPromptTemplate:
    ...
    system = dedent(
        ...
        5. <b>👥 Активные участники</b> (только top-3 по message_count):
           ...
           - ВАЖНО: Учитывай только оригинальные сообщения участников (message_count), не учитывай пересылки (forwarded_count). 
             Пересылки - это не оригинальный контент участника, а просто перепост чужих сообщений, они не отражают реальную активность.
           - ЗАПРЕЩАЕТСЯ использовать символы ▸, •, - для списков. ВСЕГДА используй <ul><li>
