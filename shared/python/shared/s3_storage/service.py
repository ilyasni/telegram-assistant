...
[C7-ID: ARCH-SHARED-001] Перемещено из api/services/s3_storage.py для соблюдения архитектурных границ
"""
...
class S3StorageService:
    ...
    async def put_media(
        ...
        try:
            ...
            # Context7: SlowDown - rate limiting от S3, требует увеличенного backoff
            # Best practice: обрабатывать SlowDown с удвоенным backoff для избежания дальнейших ограничений
            if error_code == 'SlowDown':
                logger.warning(
                    "S3 SlowDown (rate limiting) при загрузке медиа - требуется увеличенный backoff",
                    error=str(e),
                    ...
                    recommendation="Увеличить backoff delay для retry"
                )
            else:
                logger.error(
    ...
    async def put_text(
        ...
        try:
            ...
            if error_code == 'SlowDown':
                logger.warning(
                    "S3 SlowDown (rate limiting) при загрузке текста - требуется увеличенный backoff",
                    s3_key=s3_key,
                    error=str(e),
                    error_code=error_code,
                    request_id=request_id or amz_request_id,
    ...
    async def put_json(
        ...
        try:
            ...
            if error_code == 'SlowDown':
                logger.warning(
                    "S3 SlowDown (rate limiting) при загрузке JSON - требуется увеличенный backoff",
                    s3_key=s3_key,
                    error=str(e),
                    error_code=error_code,
                    request_id=request_id or amz_request_id,
    ...
    async def get_object(self, s3_key: str) -> Optional[bytes]:
        ...
        try:
            ...
            if error_code == 'SlowDown':
                ...
                logger.warning(
                    "S3 SlowDown (rate limiting) - требуется увеличенный backoff",
                    s3_key=s3_key,
                    error=str(e),
                    error_code=error_code,
                    request_id=request_id or amz_request_id,
            ...
            elif error_code == 'InternalError' or 'Internal' in str(e):
                s3_operations_total.labels(operation='get', result='error', content_type='any').inc()
                logger.warning(
