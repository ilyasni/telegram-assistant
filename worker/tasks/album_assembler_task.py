...
class AlbumAssemblerTask:
    ...
    async def _process_album_parsed(self, message_id: str, fields: Dict[str, Any]):
        ...
        post_ids = event_data.get('post_ids', [])
    ...
    async def _get_vision_results(self, post_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Используем post_enrichment для получения vision результатов
            result = await self.db.execute(
                text("""
                    SELECT 
            ...
            if row:
                ...
                labels = []
                classification = row[1]  # vision_classification
                if classification:
                    try:
                        if isinstance(classification, str):
                            classification = json.loads(classification)
                        if isinstance(classification, dict):
                            # Может быть в разных форматах: tags, labels, etc.
                ...
                return {
                    'description': row[0],  # vision_description
                    'labels': labels,
                    'is_meme': row[2] if row[2] else False,  # vision_is_meme
                    'ocr_text': row[3],  # vision_ocr_text
                    'analyzed_at': row[4]  # vision_analyzed_at
                }
        except Exception as e:
            logger.debug(
        ...
        try:
            ...
            try:
                ...
                await self.db.execute(
                    ...
                    {
                        "album_id": album_id,
                        "enrichment": json.dumps(enrichment_meta)
                    }
                )
    ...
    def _parse_event_data(self, fields: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Пытаемся извлечь из поля data/payload
        ...
        if isinstance(raw, dict):
            # Если есть вложенный payload
            if 'payload' in raw:
                return {**raw, **raw['payload']}
            return raw
        
        # Иначе - разворачиваем поля напрямую
        event_data = {}
        for key, value in fields.items():
            if key in ['post_ids', 'labels', 'tags']:
                try:
                    event_data[key] = json.loads(value) if isinstance(value, str) else value
                except Exception:
