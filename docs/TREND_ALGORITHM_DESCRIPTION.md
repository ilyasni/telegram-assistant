---
title: Описание алгоритма отслеживания и формирования трендов
date: 2025-01-20
context7: true
---
...
Система отслеживания и формирования трендов представляет собой двухуровневую архитектуру, которая работает в режиме реального времени для выявления emerging трендов и периодически подтверждает устойчивые тренды. Алгоритм основан на семантической кластеризации через векторные embeddings, временных окнах агрегации в Redis и динамических метриках для оценки значимости трендов.
...
1. **Реактивный слой (Reactive / Emerging)**
   ...
   - Назначение: Обнаружение трендов в реальном времени при появлении новых постов

2. **Стабильный слой (Stable / Confirmation)**
   - Компонент: `api/tasks/scheduler_tasks.py::trends_stable_task`
   - Триггер: Ежечасный cron (APScheduler)
   - Назначение: Подтверждение устойчивых трендов и перевод их в статус "stable"

---

## Алгоритм реактивного слоя (Reactive Pipeline)
...
**Источники данных**:
- `posts` — основной контент, метаданные, engagement метрики (views, reactions, forwards, replies)
- `post_enrichment` (kind='classify') — keywords, topics, metadata_topics
- `channels` — название канала

**Извлечение сущностей**:
- Хэштеги: паттерн `#[\w\d_]+`
...
- `grouped_id` (для дедупликации альбомов)

**Дедупликация альбомов**:
- Для постов из альбомов обрабатывается только пост с наивысшим `engagement_score`
- Остальные посты из того же альбома пропускаются
...
## Алгоритм стабильного слоя (Stable Pipeline)
...
2. Для каждого кластера:
   ...
   - Проверка порогов:
     - `freq_long >= min_freq` (по умолчанию 30)
...
**Результат**: Подтверждённые тренды сохраняются в таблицу `trends_detection` для долгосрочного хранения и анализа
...
**Stable Task**:
- `trend_stable_min_freq` (default: 30) — минимальная частота для stable статуса
- `trend_stable_min_sources` (default: 3) — минимальное количество источников
- `trend_stable_min_burst` (default: 1.5) — минимальный burst score
...
- `trend_detection_ratio` — гистограмма burst ratio для диагностики
- `trend_detection_coherence` — гистограмма coherence score
- `trend_detection_source_diversity` — гистограмма source diversity
- `trend_detection_threshold_reasons{reason}` — причины отклонения трендов (ratio_too_low, source_diversity_too_low, coherence_too_low, cooldown, all_passed)
...
6. **Дедупликация альбомов**: Обработка только лучшего поста из альбома
...
7. **Threshold Tuner Agent**: Автоматическая оптимизация порогов на основе истории

---

## Заключение

Алгоритм обнаружения трендов использует комбинацию семантической кластеризации (embeddings + Qdrant ANN), временных окон агрегации (Redis), динамических метрик (burst, coherence, novelty) и LLM-обогащения для генерации человекочитаемых карточек трендов. Система работает в реальном времени (reactive) и подтверждает устойчивые тренды через стабильный пайплайн (stable), обеспечивая баланс между скоростью обнаружения и качеством результатов.
